...
Full product spec: `docs/core/product-spec.md`
Architecture: `docs/core/architecture.md`
Constitution v1: `docs/core/constitution-v1.md`
Staged build plan: `docs/core/implementation-plan.md`
...
2. Read `docs/core/current-status.md` — what works, what is stubbed, what is blocked
3. Read `docs/core/session-handoff.md` — architectural invariants and what not to touch
...
If the session was interrupted mid-task, read `docs/core/current-status.md` first — it records the last known passing state.

---

## Stage Discipline

Always check which stage is active before implementing. Do not implement Stage 2 features while Stage 1 is incomplete. After each stage, run the verification commands listed in `docs/core/implementation-plan.md` and confirm they pass before proceeding.

Current stage: **Stage 6.4 complete (Seamless Conversation Entry + Observer-first Side Action Rail)** — UI / UX only. No xAI, Anthropic, or X API calls; no Supabase writes beyond existing user actions; no service-role. Opening a debate from the Conversation Gallery defaults to Observer / read mode (no "choose side" modal); existing participants keep their actual side. New `ArgumentSideActionRail.tsx` is collapsed by default for observers; expanded set is `Watch · Join For · Join Against · Ask source · Open timeline · Share`. Participant on another bubble → `Reply · Disagree · Ask source · Ask quote · Split branch · Flag · Qualifiers`. Own bubble → `Qualifiers · Request deletion` only (no edit, no disagree, no flag, no score). New `deriveConversationEntryHint(card)` pre-activates the right message per bucket (`needs_rebuttal → root`, `source_chain_fight → first open challenge`, others → latest) and shows a one-line "micro-moment" prompt above the body. New section grouping `groupGalleryCardsBySection` for the entry screen: `Jump into a live dispute · Needs first rebuttal · Source trail fights · Hot but unresolved · Easy first move · My rooms`. Gallery action labels now `Observe → / Continue → / Open →` (no more "Tap to join" on the gallery; that copy is reserved for the explicit in-rail join). New `gameCopy.toPlainLanguage` maps internal codes (topic_satisfaction_lexical, source_chain, anti_amplification, evidence_debt, platform_support_warning, validation_failed_after_retries, max_depth_reached, synthesis_ready, submit_failed, observer, moderator, etc.) to normal-user prose. **1805 tests / 70 suites passing** (+33 seamless-entry tests). Typecheck + lint clean. See `docs/seamless-conversation-entry.md`.

Earlier completed stage: **Stage 6.3 complete (Conversation Gallery + horizontal argument timeline UX)** — UI / model only. New pure model `src/features/debates/conversationGalleryModel.ts` (cards, dedupe, 11 buckets, heat, temperament, signals, search, sort, pagination). New screen `ConversationGalleryScreen.tsx` replaces the row-per-debate table; collapses `[xai-adv …]` / `[ai-corpus …]` / `[stress …]` duplicates visually (DB rows untouched). New `ConversationMiniTimeline.tsx` renders one dot per move on a single horizontal baseline. `argumentGameSurfaceModel.computeLane` patched so the first child of a parent continues on the parent's lane (no more diagonal scatter). `listArgumentsForDebateIds(ids)` adds a single batched `.in()` query for gallery loads; `useGalleryArguments` hook wraps it. Future-voting placeholders reserved (`voteScorePreview`, `winnerPreview`, `promotedArgumentCount`). **1772 tests / 69 suites passing** (+50 gallery model + 6 mini-timeline helper). Typecheck + lint clean. No xAI, Anthropic, or X API calls; no Supabase writes beyond existing user actions; no service-role; no DB migration. **Parallel verification this session**: 100-harvest + 50-scenario corpus produced 320/320 posted across 32 rooms with 8 distinct Anthropic-chosen disagreement axes (82% overridden vs round-robin fallback). See `docs/conversation-gallery-ux.md`.

Earlier completed stage: **Stage 6.1.9 follow-up complete (dry JSONL contract stabilized; M1/M2 seed exemption tests)** — On top of Stage 6.2 UX rescue (timeline map / sidecar / advisory validation). The harvester now writes to `<runId>-xai-adversarial-harvest.jsonl` so it no longer collides with the runner's `…-semantic-corpus.jsonl`. The runner's dry mode emits the full required event set plus 4 new granular events (`bot_assignment`, `move_prompt_built`, `move_rendered`, `move_validated`); existing `bot_move_render` retained for back-compat. Every event still carries `skillGate` with both 16-hex skill hashes. New tests in `__tests__/seedM1M2NoKeywordStuffing.test.ts` (7 tests) prove the local shared engine never rejects xAI-derived M1/M2 seeds for missing keyword overlap — Stage 6.2 already converted OFF_TOPIC and PARENT_NONRESPONSIVE to advisory. **1671 tests / 65 suites passing**, typecheck + lint clean. **Tiny live verification done this session**: 1 xAI harvest (5 sources / 30 replies scanned / 4 usable / 3 synthetic), 1 Anthropic corpus run (14 calls, ~49k input + ~1.6k output tokens), 5 dev debate rooms created, 13/18 moves posted via `submit-argument`. The 5 rejections all trace to a single deployed-Edge-Function issue: the deployed `submit-argument` still hard-blocks on the pre-Stage-6.2 OFF_TOPIC rule. **Operator action**: run `npx supabase functions deploy submit-argument --linked` to pick up the advisory change. **Do NOT run 100-harvest / 50-scenario corpus until that deploy lands** — the rejection rate is gated on it. No service-role, no direct insert, no `.env*` touched. See `docs/core/current-status.md`.
