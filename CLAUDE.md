...
Current stage: **Stage 6.1.2.4b complete** — Bot fixture runner repaired and first end-to-end live fixture run achieved: `sports-play-in` posted 7/7 moves via normal Supabase auth + `submit-argument` (room `62305b8b-c11e-41a6-81b8-4c95daf73d2c`). `admin-users` ACTIVE v1, `submit-argument` ACTIVE v1, both deployed to hosted project. Runner now preserves real HTTP status from `FunctionsHttpError.context.status` (no more `failed_500` collapse), records `errorDetail`, skips children when parent did not post, and maps persona `neutral`→participant `moderator`. Fixture sports-play-in updated for constitution transitions (m6 concession→m4 claim; m7 synthesis→m6 concession), parent-overlap vocabulary, and the concession marker required by the `concession_integrity` rail. Runner no longer passes `scenario.notes` as debate description. 803 tests / 27 suites passing, lint clean, typecheck clean. Next: live admin browser smoke (A–H), run remaining bot fixtures, UX triage of generated bot room.
