/**
 * Adapters that convert snake_case DB rows (from Supabase) into the
 * camelCase types expected by the rules engine.
 *
 * Never pass raw Supabase rows directly to evaluateArgumentDraft.
 * Always run them through these adapters first.
 */
import type {
  ConstitutionVersion,
  ConstitutionRule,
...
// ── Param key normalizer ──────────────────────────────────────

/**
 * Normalizes snake_case param keys to camelCase equivalents so the engine
 * can use consistent key names regardless of where the rule was loaded from.
 * Preserves original snake_case keys to avoid breaking any code that reads them.
 */
