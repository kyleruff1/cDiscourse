 ...
 * `src/features/cutoverHealthAlerts/cutoverHealthAlertModel.ts`. Behavior-parity
 * copy; the src/ original has NO imports (fully self-contained pure-TS), so this
 * twin is a verbatim copy with only the header changed. Server (Deno / Edge) side.
 *
 * Why a twin: two Edge functions imported this module straight from
 * `../../../src/features/...`, a path OUTSIDE the `supabase/functions/` tree —
 * `cutover-health-monitor` (full module) and `admin-classifier-health`
 * (`containsForbiddenSubstring` only). Cross-tree `src/` imports can fail to
 * resolve at deploy/boot time. This `_shared` twin keeps both functions
 * self-contained under `supabase/functions/`.
 *
 ...
 *   - The admin-classifier-health Edge Function (the `containsForbiddenSubstring`
 *     defensive scrub).
 ...
 * Used both at runtime (defensive scrubber on outgoing payloads) and in tests
 * (assert that every classifier output is sentinel-free).
 *
 * Categories:
 ...
 * that classifier outputs are clean; consumers add a final defensive scrub.
 *
 ...
 * to catch ANY shape of the forbidden tokens. Used by tests + consumers.
 */
