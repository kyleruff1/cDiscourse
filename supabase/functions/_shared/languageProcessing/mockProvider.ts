 ...
 * Pure TypeScript — no network calls, no Deno.env, deterministic output.
 * Used for tests, local dev, and as a fallback when AI_LANGUAGE_PROCESSING_PROVIDER=mock.
 *
 * MIRROR: src/features/languageProcessing/mockProvider.ts
 * Only difference: import paths use .ts extensions for Deno compatibility.
...
export function runMockLanguageProcessing(
  ...
  // Reset lastIndex since FILLER_PATTERN has /g flag
