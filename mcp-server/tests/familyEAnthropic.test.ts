/**
 * MCP-SERVER-006-FAMILY-E — runAnthropicFamilyEClassifier mock-fetch tests.
 *
 ...
 *   - Family E MAX_TOKENS=1500 is honored (call-args sniff)
 */
...
const SAMPLE_VALID_PACKET = {
  ...
  evidenceSpan: { slippery_slope_reasoning_present: null },
  modelInfo: {
    provider: 'mcp',
...
function buildRequest(): ValidatedFamilyERequest {
  return {
    schemaVersion: MCP_BOOLEAN_OBSERVATION_SCHEMA_VERSION,
    nodeId: 'node-e-1',
    parentNodeId: null,
    currentText: 'If we permit this, agencies will define acceptable speech, then expand the categories, then arrive at full suppression.',
    parentText: null,
...
Deno.test('runAnthropicFamilyEClassifier: happy path returns parsed packet', async () => {
  await withFakeKey(async () => {
    ...
    const result = await runAnthropicFamilyEClassifier(buildRequest(), 'req-e-1', mockFetch);
...
Deno.test('runAnthropicFamilyEClassifier: key_missing when ANTHROPIC_API_KEY unset', async () => {
  await withNoKey(async () => {
    const result = await runAnthropicFamilyEClassifier(buildRequest(), 'req-e-2');
...
Deno.test('runAnthropicFamilyEClassifier: HTTP 429 returns rate_limited', async () => {
  await withFakeKey(async () => {
    const mockFetch: typeof fetch = async () => {
      return await Promise.resolve(new Response('rate limited', { status: 429 }));
    };
    const result = await runAnthropicFamilyEClassifier(buildRequest(), 'req-e-3', mockFetch);
    assertEquals(result.ok, false);
    if (!result.ok) {
...
Deno.test('runAnthropicFamilyEClassifier: HTTP 500 returns api_error', async () => {
  await withFakeKey(async () => {
    const mockFetch: typeof fetch = async () => {
      return await Promise.resolve(new Response('boom', { status: 500 }));
    };
    const result = await runAnthropicFamilyEClassifier(buildRequest(), 'req-e-4', mockFetch);
    assertEquals(result.ok, false);
    if (!result.ok) {
...
Deno.test('runAnthropicFamilyEClassifier: TimeoutError returns model_timeout', async () => {
  await withFakeKey(async () => {
    const mockFetch: typeof fetch = async () => {
    ...
    const result = await runAnthropicFamilyEClassifier(buildRequest(), 'req-e-5', mockFetch);
    assertEquals(result.ok, false);
    if (!result.ok) {
...
Deno.test('runAnthropicFamilyEClassifier: non-JSON response returns parse_failure', async () => {
  await withFakeKey(async () => {
    const mockFetch: typeof fetch = async () => {
      return await Promise.resolve(new Response('plain text not json', { status: 200 }));
    };
    const result = await runAnthropicFamilyEClassifier(buildRequest(), 'req-e-6', mockFetch);
    assertEquals(result.ok, false);
    if (!result.ok) {
...
Deno.test('runAnthropicFamilyEClassifier: model text without JSON object returns parse_failure', async () => {
  await withFakeKey(async () => {
    const mockFetch: typeof fetch = async () => {
      return await Promise.resolve(
        new Response(
    ...
    const result = await runAnthropicFamilyEClassifier(buildRequest(), 'req-e-7', mockFetch);
    assertEquals(result.ok, false);
    if (!result.ok) {
      assertEquals(result.reason, 'parse_failure');
    }
...
Deno.test('runAnthropicFamilyEClassifier: API key NEVER appears in any log line (success path)', async () => {
  await withFakeKey(async () => {
    const lines: string[] = [];
    ...
    try {
      ...
      await runAnthropicFamilyEClassifier(buildRequest(), 'req-e-log-1', mockFetch);
...
Deno.test('runAnthropicFamilyEClassifier: API key NEVER appears in any log line (failure path)', async () => {
  await withFakeKey(async () => {
    const lines: string[] = [];
    _setLogSinkForTesting((line) => lines.push(line));
    try {
      ...
      await runAnthropicFamilyEClassifier(buildRequest(), 'req-e-log-2', mockFetch);
      for (const line of lines) {
        if (line.includes(FAKE_KEY)) {
          throw new Error(`API key appeared in log: ${line}`);
        }
...
Deno.test('runAnthropicFamilyEClassifier: logs are tagged with classify_argument_boolean_observations tool', async () => {
  await withFakeKey(async () => {
    const lines: string[] = [];
    _setLogSinkForTesting((line) => lines.push(line));
    try {
      ...
      await runAnthropicFamilyEClassifier(buildRequest(), 'req-e-tag-1', mockFetch);
...
Deno.test('runAnthropicFamilyEClassifier: MAX_TOKENS=1500 is sent to Anthropic API (no bump)', async () => {
  // Family E uses MAX_TOKENS=1500 (matches Family A/B/C). Family D bumped to
  // 1800 for the 19-key Subset; Family E stays at 1500 per design §2.
  await withFakeKey(async () => {
    ...
    await runAnthropicFamilyEClassifier(buildRequest(), 'req-e-budget-1', mockFetch);
