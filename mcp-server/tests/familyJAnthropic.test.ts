/**
 * MCP-SERVER-011-FAMILY-J — runAnthropicFamilyJClassifier mock-fetch tests.
 *
 ...
 *   - Family J MAX_TOKENS=1500 is honored (call-args sniff; no bump per design §4)
 */
...
const SAMPLE_VALID_PACKET = {
  ...
  confidence: { shifts_to_person_or_intent: 'high' },
  evidenceSpan: { shifts_to_person_or_intent: null },
  modelInfo: {
    provider: 'mcp',
...
function buildRequest(): ValidatedFamilyJRequest {
  return {
    schemaVersion: MCP_BOOLEAN_OBSERVATION_SCHEMA_VERSION,
    nodeId: 'node-j-1',
    parentNodeId: null,
...
Deno.test('runAnthropicFamilyJClassifier: happy path returns parsed packet', async () => {
  await withFakeKey(async () => {
    ...
    const result = await runAnthropicFamilyJClassifier(buildRequest(), 'req-j-1', mockFetch);
...
Deno.test('runAnthropicFamilyJClassifier: key_missing when ANTHROPIC_API_KEY unset', async () => {
  await withNoKey(async () => {
    const result = await runAnthropicFamilyJClassifier(buildRequest(), 'req-j-2');
...
Deno.test('runAnthropicFamilyJClassifier: HTTP 429 returns rate_limited', async () => {
  await withFakeKey(async () => {
    const mockFetch: typeof fetch = async () => {
      return await Promise.resolve(new Response('rate limited', { status: 429 }));
    };
    const result = await runAnthropicFamilyJClassifier(buildRequest(), 'req-j-3', mockFetch);
    assertEquals(result.ok, false);
    if (!result.ok) {
...
Deno.test('runAnthropicFamilyJClassifier: HTTP 500 returns api_error', async () => {
  await withFakeKey(async () => {
    const mockFetch: typeof fetch = async () => {
      return await Promise.resolve(new Response('boom', { status: 500 }));
    };
    const result = await runAnthropicFamilyJClassifier(buildRequest(), 'req-j-4', mockFetch);
    assertEquals(result.ok, false);
    if (!result.ok) {
...
Deno.test('runAnthropicFamilyJClassifier: TimeoutError returns model_timeout', async () => {
  await withFakeKey(async () => {
    const mockFetch: typeof fetch = async () => {
    ...
    const result = await runAnthropicFamilyJClassifier(buildRequest(), 'req-j-5', mockFetch);
    assertEquals(result.ok, false);
    if (!result.ok) {
...
Deno.test('runAnthropicFamilyJClassifier: non-JSON response returns parse_failure', async () => {
  await withFakeKey(async () => {
    const mockFetch: typeof fetch = async () => {
      return await Promise.resolve(new Response('plain text not json', { status: 200 }));
    };
    const result = await runAnthropicFamilyJClassifier(buildRequest(), 'req-j-6', mockFetch);
    assertEquals(result.ok, false);
    if (!result.ok) {
...
Deno.test('runAnthropicFamilyJClassifier: model text without JSON object returns parse_failure', async () => {
  await withFakeKey(async () => {
    const mockFetch: typeof fetch = async () => {
      return await Promise.resolve(
        new Response(
    ...
    const result = await runAnthropicFamilyJClassifier(buildRequest(), 'req-j-7', mockFetch);
    assertEquals(result.ok, false);
    if (!result.ok) {
      assertEquals(result.reason, 'parse_failure');
    }
...
Deno.test('runAnthropicFamilyJClassifier: API key NEVER appears in any log line (success path)', async () => {
  await withFakeKey(async () => {
    const lines: string[] = [];
    ...
    try {
      ...
      await runAnthropicFamilyJClassifier(buildRequest(), 'req-j-log-1', mockFetch);
...
Deno.test('runAnthropicFamilyJClassifier: API key NEVER appears in any log line (failure path)', async () => {
  await withFakeKey(async () => {
    const lines: string[] = [];
    _setLogSinkForTesting((line) => lines.push(line));
    try {
      ...
      await runAnthropicFamilyJClassifier(buildRequest(), 'req-j-log-2', mockFetch);
      for (const line of lines) {
        if (line.includes(FAKE_KEY)) {
          throw new Error(`API key appeared in log: ${line}`);
        }
...
Deno.test('runAnthropicFamilyJClassifier: logs are tagged with classify_argument_boolean_observations tool', async () => {
  await withFakeKey(async () => {
    const lines: string[] = [];
    _setLogSinkForTesting((line) => lines.push(line));
    try {
      ...
      await runAnthropicFamilyJClassifier(buildRequest(), 'req-j-tag-1', mockFetch);
...
Deno.test('runAnthropicFamilyJClassifier: MAX_TOKENS=1500 is sent to Anthropic API (no bump per design §4)', async () => {
  await withFakeKey(async () => {
    ...
    await runAnthropicFamilyJClassifier(buildRequest(), 'req-j-budget-1', mockFetch);
