 ...
 *   - upstream `familyG.ts` has exactly 30 rawKey declarations
 *     (5 auto_metadata + 7 lifecycle + 18 ai_classifier)
 *   - the server-side subset contains exactly the 18 ai_classifier-source
 *     entries; the 12 deterministic rawKey declarations are absent from
 *     `FAMILY_G_RAW_KEYS`
 *
 * SOURCE COUNT NOTE (design §A.1.0): the upstream `familyG.ts` file-header
 * comment is STALE — it says "29 entries … ai_classifier (8)" but the actual
 * array has 30 frozen entries with 18 ai_classifier declarations. The binding
 * contract is the actual `source: 'ai_classifier'` literals in the code (18),
 * NOT the stale header. This test asserts exactly 18 ai_classifier
 * declarations and 30 total. This card does NOT fix the upstream header (it is
 * in `src/`, out of scope per intent §6).
 *
...
Deno.test('familyGKeysParity: upstream familyG.ts has exactly 30 rawKey declarations (5 + 7 + 18)', async () => {
  const upstream = await Deno.readTextFile(UPSTREAM_FAMILY_G_PATH);
  ...
  // Family G upstream MUST have 30 declarations (design §A.1.0 binding;
  // the stale header says 29, but the actual array has 30).
  if (upstreamDeclarations.length !== 30) {
    throw new Error(
      `Upstream familyG.ts has ${upstreamDeclarations.length} rawKey declarations; expected 30. Drift detected.`,
    );
  }
});

Deno.test('familyGKeysParity: upstream familyG.ts contains all 18 ai_classifier-subset rawKey literals', async () => {
  const upstream = await Deno.readTextFile(UPSTREAM_FAMILY_G_PATH);
...
Deno.test('familyGKeysParity: upstream ai_classifier-source declarations match server-side subset (set equality, exactly 18)', async () => {
  const upstream = await Deno.readTextFile(UPSTREAM_FAMILY_G_PATH);
  ...
  assertEquals(
    ...
    18,
    `Upstream familyG.ts ai_classifier entries: expected 18, got ${aiClassifierKeys.length}`,
  );
