/**
 * MCP-SERVER-004-FAMILY-C — Family C fixture provider for offline smoke testing.
 *
 * Activated when `MCP_SERVER_USE_FIXTURE_PROVIDER=true`. Loads the canonical
 * Family C response from
 * `fixtures/classify-argument-boolean-observations.family-c-canonical-response.json`
 * and returns it as the model "response". No real Anthropic call, no token
 * consumption — used by the smoke script (Checks 12 + 13) to verify the
...
export type FamilyCFixtureResult =
  | { ok: true; value: Record<string, unknown> }
  | { ok: false; reason: 'fixture_load_failed' };

/**
 * Load the Family C canonical response fixture. Returns the parsed JSON
 ...
 */
export async function loadFixtureFamilyCPacket(): Promise<FamilyCFixtureResult> {
  try {
    const text = await Deno.readTextFile(FIXTURE_PATH);
    const parsed = JSON.parse(text);
    if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
      log('error', 'family_c_fixture_provider_invalid_shape', {
    ...
    log('error', 'family_c_fixture_provider_threw', {
      reason: 'fixture_load_failed',
      status: 'failure',
