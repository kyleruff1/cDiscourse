/**
 * MCP-SERVER-007-FAMILY-F — Family F boolean-observation Anthropic orchestrator.
 *
 * Wraps the shared `callAnthropic` skeleton with Family F-specific prompt
 * construction and `tool: 'classify_argument_boolean_observations'`
 * logging. Returns the parsed JSON packet for the tool handler to validate.
 *
 * Doctrine:
 *   - cdiscourse-doctrine §7 — server-side Deno only.
 *   - cdiscourse-doctrine §6 — ANTHROPIC_API_KEY never logged; reaches
 *     network via x-api-key inside callAnthropic.
 *   - The structural validation + ban-list scan happen in the tool handler,
 *     NOT here.
 *
 * Per design §2: MAX_TOKENS=1500 (matches Family A/B/C/E; NO bump). ~310
 * token headroom on 14 keys × ~85 tokens per key (lighter load than
 * Family E's 16 keys with ~60 token headroom).
 */
import { callAnthropic } from './anthropicCall.ts';
import type { AnthropicCallResult } from './anthropicCall.ts';
import {
  FAMILY_F_SYSTEM_PROMPT,
  FAMILY_F_MAX_TOKENS,
  FAMILY_F_TEMPERATURE,
  buildFamilyFUserPrompt,
} from './familyFPrompt.ts';
import type { ValidatedFamilyFRequest } from './familyFPrompt.ts';

/**
 * Run the Family F boolean-observation classifier call against Anthropic.
 *
 * @param request Validated Family F request.
 * @param requestId Server-generated correlation id.
 * @param fetchImpl Optional override for unit tests.
 */
export async function runAnthropicFamilyFClassifier(
  request: ValidatedFamilyFRequest,
  requestId: string,
  fetchImpl: typeof fetch = fetch,
): Promise<AnthropicCallResult> {
  const userPrompt = buildFamilyFUserPrompt(request);
  return await callAnthropic({
    system: FAMILY_F_SYSTEM_PROMPT,
    userPrompt,
    maxTokens: FAMILY_F_MAX_TOKENS,
    temperature: FAMILY_F_TEMPERATURE,
    toolNameForLogging: 'classify_argument_boolean_observations',
    requestId,
    perToolTimeoutMs: request.timeoutMs,
    fetchImpl,
  });
}
