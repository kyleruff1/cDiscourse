/**
 * MCP-SERVER-007-FAMILY-F — Family F fixture provider for offline smoke testing.
 *
 * Activated when `MCP_SERVER_USE_FIXTURE_PROVIDER=true`. Loads the canonical
 * Family F response from
 * `fixtures/classify-argument-boolean-observations.family-f-canonical-response.json`
 * and returns it as the model "response". No real Anthropic call, no token
 * consumption — used by the smoke script (Checks 18 + 19) to verify the
...
export type FamilyFFixtureResult =
  | { ok: true; value: Record<string, unknown> }
  | { ok: false; reason: 'fixture_load_failed' };

/**
 * Load the Family F canonical response fixture. Returns the parsed JSON
 ...
 */
export async function loadFixtureFamilyFPacket(): Promise<FamilyFFixtureResult> {
  try {
    const text = await Deno.readTextFile(FIXTURE_PATH);
    const parsed = JSON.parse(text);
    if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
      log('error', 'family_f_fixture_provider_invalid_shape', {
    ...
    log('error', 'family_f_fixture_provider_threw', {
      reason: 'fixture_load_failed',
      status: 'failure',
