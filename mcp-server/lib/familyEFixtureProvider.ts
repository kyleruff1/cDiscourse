/**
 * MCP-SERVER-006-FAMILY-E — Family E fixture provider for offline smoke testing.
 *
 * Activated when `MCP_SERVER_USE_FIXTURE_PROVIDER=true`. Loads the canonical
 * Family E response from
 * `fixtures/classify-argument-boolean-observations.family-e-canonical-response.json`
 * and returns it as the model "response". No real Anthropic call, no token
 * consumption — used by the smoke script (Checks 16 + 17) to verify the
...
export type FamilyEFixtureResult =
  | { ok: true; value: Record<string, unknown> }
  | { ok: false; reason: 'fixture_load_failed' };

/**
 * Load the Family E canonical response fixture. Returns the parsed JSON
 ...
 */
export async function loadFixtureFamilyEPacket(): Promise<FamilyEFixtureResult> {
  try {
    const text = await Deno.readTextFile(FIXTURE_PATH);
    const parsed = JSON.parse(text);
    if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
      log('error', 'family_e_fixture_provider_invalid_shape', {
    ...
    log('error', 'family_e_fixture_provider_threw', {
      reason: 'fixture_load_failed',
      status: 'failure',
