/**
 * MCP-SERVER-003-FAMILY-B — Family B fixture provider for offline smoke testing.
 *
 * Activated when `MCP_SERVER_USE_FIXTURE_PROVIDER=true`. Loads the canonical
 * Family B response from
 * `fixtures/classify-argument-boolean-observations.family-b-canonical-response.json`
 * and returns it as the model "response". No real Anthropic call, no token
 * consumption — used by the smoke script (Checks 10 + 11) to verify the
...
export type FamilyBFixtureResult =
  | { ok: true; value: Record<string, unknown> }
  | { ok: false; reason: 'fixture_load_failed' };

/**
 * Load the Family B canonical response fixture. Returns the parsed JSON
 ...
 */
export async function loadFixtureFamilyBPacket(): Promise<FamilyBFixtureResult> {
  try {
    const text = await Deno.readTextFile(FIXTURE_PATH);
    const parsed = JSON.parse(text);
    if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
      log('error', 'family_b_fixture_provider_invalid_shape', {
    ...
    log('error', 'family_b_fixture_provider_threw', {
      reason: 'fixture_load_failed',
      status: 'failure',
