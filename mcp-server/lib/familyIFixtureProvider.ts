/**
 * MCP-SERVER-010-FAMILY-I — Family I fixture provider for offline smoke testing.
 *
 * Activated when `MCP_SERVER_USE_FIXTURE_PROVIDER=true`. Loads the canonical
 * Family I response from
 * `fixtures/classify-argument-boolean-observations.family-i-canonical-response.json`
 * and returns it as the model "response". No real Anthropic call, no token
 * consumption — used by the smoke script (Checks 24 + 25) to verify the
...
/**
 * Load the Family I canonical response fixture. Returns the parsed JSON
 ...
 */
export async function loadFixtureFamilyIPacket(): Promise<FamilyIFixtureResult> {
  try {
    const text = await Deno.readTextFile(FIXTURE_PATH);
    const parsed = JSON.parse(text);
    if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
      log('error', 'family_i_fixture_provider_invalid_shape', {
    ...
    log('error', 'family_i_fixture_provider_threw', {
      reason: 'fixture_load_failed',
      status: 'failure',
