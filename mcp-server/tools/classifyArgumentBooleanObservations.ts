 ...
 * has `productionEnabled: false, adminValidationEnabled: true`. The MCP
 * server classifier handles the 21-key ai_classifier Subset (the 12
 * deterministic auto_metadata + lifecycle keys are excluded; requesting any
 * of them returns unsupported_rawKey). Because 21 exceeds the per-response
 * cap of 20, the Edge classifier splits Family G into 2 batches (16 + 5) and
 * merges the results into one run; each batch is served here as a normal
 * single-family request. The resolution<->verdict doctrine
