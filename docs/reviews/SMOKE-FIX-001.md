...
| `docs/core/current-status.md` 3-line footnote (§5.5) | Pass | Adds a new SMOKE-FIX-001 section ABOVE the existing MCP-018 section, with the 3 line footnote (log format + test delta + operator follow-up). Does NOT modify CLAUDE.md (correctly — this card does not bump the stage line). |
...
Baseline before card (from `main`'s `docs/core/current-status.md` header): **9056 tests / 328 suites**.
