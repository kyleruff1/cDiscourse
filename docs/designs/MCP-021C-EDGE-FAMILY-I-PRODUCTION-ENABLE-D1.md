...
- **Stale-test updates from I joining the production roster — HUNT count-drift beyond the obvious set (MCP-H-002 / PR #559 lesson).** PR #559 found drift in files beyond the obvious sibling suites; full Jest is the only reliable detector. Named candidates to verify (re-baseline ONLY if they assert a roster-derived count):
