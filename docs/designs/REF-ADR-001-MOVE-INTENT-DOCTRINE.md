...
- **Channels are an authoritative relation source.** REF-001's `relationToParent` derivation may read the selected `MoveChannel` (`BuildOpenIssueInput.selectedChannel`, `docs/designs/REF-001-DISAGREEMENT-CONTRACT-REFEREE-CARD.md:478`) above the stored `ArgumentType` — the channel is the user's stated move intent, the stored type is the fallback.
