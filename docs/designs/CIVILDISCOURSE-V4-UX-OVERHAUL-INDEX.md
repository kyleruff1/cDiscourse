...
| UX-MEDIATOR-001 | #682 | One-state-per-node derivation (single source; reuse shipped deriveMediatorBoardState) | P0 | L | pure-TS | No | Eligible after reviewer PASS (pure model) | UX-DESIGN-PACKAGE-001 | **Implemented** — `decidePointState` refactored to candidate-set + Gate A + ordered picker (fixes D1 impasse-too-low / D2 narrowed-too-high); added `V4MediatorStateCode` + `V4_PRIMARY_STATE_PRIORITY` + total 13→9 `V4_DISPLAY_STATE_BY_CODE` + `v4DisplayStateFor` helper; `missing_mechanism` label → "Missing link"; "Definition needed" rename deferred to UX-MEDIATOR-004; +30 tests. Pure-TS only; engine remains sole gate. Ready for review. |
| UX-MEDIATOR-002 | #683 | One primary state chip · node markup (collapse NodeLabelStrip + MediatorNodeMarker) | P0 | M | UI-model | No | Eligible after reviewer PASS | UX-MEDIATOR-001 | **Implemented** — default node view collapsed to ONE primary state chip (MediatorNodeMarker projected through v4DisplayStateFor; O-1); NodeLabelStrip UNMOUNTED from default view, its Observation/Allegation content relocated into the existing Inspect overlay (NodeLabelInspectGroups); chip-adjacent Inspect caret (role button, ≥44×44; O-2); new read-only MediatorNodeInspectDetail block in Inspect (state label + helper + next-move; O-3); rail untouched (O-4 → UX-MEDIATOR-005); "Definition needed" rename deferred (UX-MEDIATOR-004); sensitive composer-only marks on neither surface; +30 tests; web:build/typecheck/lint clean. Ready for review. |
...
| UX-FEEDBACK-001 | #690 | Clarity feedback ratings (clarity not popularity; folds UX-CLARITY-RATING-001 intent) | P1 | M | UI-copy | No | Eligible after reviewer PASS | UX-MEDIATOR-002 | Implemented — display slice (STATIC current-state notes; no gamification / no transition language / no rating persistence). Rating-persistence chip-set deferred (GATE-C) + transition moments deferred (UX-FEEDBACK-DELTA-001). |
