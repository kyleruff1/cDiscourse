...
| `app.json` `plugins` array committed on this branch alongside the pure-TS core              | **Deferred to the runbook (option a, orchestrator ruling 2026-08-04).** The design originally classified the addition as inert-without-install; direct verification proved the Expo config resolver walks `expo.plugins` eagerly at every `getConfig()` and calls `require.resolve('expo-audio/app.plugin.js')`, raising `PluginError` in `expo export --platform web` when the dep is absent. The plugin tuple + the `voice005AppJsonContract.test.ts` therefore move into the runbook as an operator step (install + `app.json` edit + contract-test add in the same coordinated action). Doctrine copy stays pinned by prose in design §8.3 + runbook Appendix A. | Orchestrator ruling 2026-08-04; §13 |
...
| `src/features/voice/visualizer/index.ts`                                                            | Barrel. Exports the four public symbols: `bucketsToPathSegments`, `pathSegmentsToSvgD`, `applyPathSegmentsToSkiaPath`, `quantizeBucketsForRender` (plus the three type re-exports). No `bucketsToPath` composition helper — early drafts named one; final barrel omits it since no consumer needs a pre-composed shortcut. | commit_now                    |
| `__tests__/bucketsToPathSegments.test.ts`                                                           | 25+ scenarios per the test matrix (§7). Determinism, geometry, defensive clamps, live-vs-finalized parity, cross-module identity. **Repo-root `__tests__/` per the VOICE-004 convention.**                | commit_now                    |
| `__tests__/svgPathAdapter.test.ts`                                                                  | Golden `d`-string fixtures for empty / single / two-zero; `toFixed(3)` locale-safety proof; byte-identity across module re-imports. **Repo-root `__tests__/` per VOICE-004 convention.**                  | commit_now                    |
| `__tests__/skiaPathAdapter.test.ts`                                                                 | Fake `SkiaPathLike` records `['moveTo'|x|y]` / `['lineTo'|x|y]` / `['close']`; parametric parity assertion with the SVG-joiner over the canonical shape set. **Repo-root `__tests__/`.**                 | commit_now                    |
| `__tests__/quantizeBucketsForRender.test.ts`                                                        | Idempotence property test over the 256-lattice; `clamp01` boundary; parity with VOICE-004's `quantize8bit` on shared inputs. **Repo-root `__tests__/`.**                                                 | commit_now                    |
| `__tests__/voice005ForbiddenInferenceGuard.test.ts`                                                 | Source-scan of `src/features/voice/visualizer/**/*.ts` for the VOICE-004 forbidden lexicon + visualizer bans (§6) + module-level mutability bans + trig/non-basic-arith bans + Skia SVG-parser ban. **Repo-root `__tests__/`.** | commit_now                    |
| `src/features/voice/visualizer/__fixtures__/voice005ForbiddenInferenceGuard.positiveControl.ts.txt` | Firing positive-control fixture. Contains banned tokens under 5 categories (visualizer-specific, purity, non-basic-arith, Skia SVG-parser, inherited lexicon). `.ts.txt` so it does not compile. **Sibling of the module, not under `__tests__/` — matches VOICE-004 fixture placement.** | commit_now                    |
| `__tests__/reducerIntegration.test.ts` (dropped)                                                    | **Reclassified — no separate file needed.** The integration scenarios (§7.2 #17-21) are covered at the pure-function boundary: `__tests__/bucketsToPathSegments.test.ts` proves INV-A byte-equal double-call + INV-B live-vs-finalized parity across the 256-level lattice, and VOICE-004's `waveformSessionMachine.test.ts` already covers the reducer terminal-state IGNORE identity and `USER_RESET → initialState` behavior. A wrapper suite that re-runs the reducer to feed the renderer would be a mock of behavior both suites already prove independently. | covered_by_pure_boundary_tests + VOICE-004 reducer suite |
| `src/features/voice/visualizer/__tests__/appJsonPluginContract.test.ts`                             | Parses `app.json`; asserts `plugins[0] === ['expo-audio', {microphonePermission: <exact doctrine copy>}]`; catches wording drift + typo (`microphonePermissions`, `micPermission`) before prebuild.       | operator_step_only (per §13; runbook step 3) |
| `app.json`                                                                                          | Operator adds `plugins` array (single `expo-audio` tuple entry with the doctrine mic-permission copy) in the SAME commit as the `npx expo install expo-audio` step. Pre-install addition raises `PluginError` in `expo export --platform web`; see §13 for verification. | operator_step_only (per §13; runbook step 3) |
...
### 7.2 Cross-module integration tests — reclassified as covered by pure-boundary tests

**Reviewer nit 1 (2026-08-04): §7.2 scenarios 17-21 have no dedicated `reducerIntegration.test.ts` file on the branch. They are functionally satisfied by the pure-function boundary tests in `__tests__/bucketsToPathSegments.test.ts` (INV-A byte-equal double call + INV-B live-vs-finalized parity across every point on the 256-level lattice) plus VOICE-004's `waveformSessionMachine.test.ts` (already covers reducer terminal-state IGNORE identity, `USER_RESET → initialState`, and `AVAILABILITY_LOST` reference preservation). A wrapper suite that re-runs the reducer to feed the renderer would be a mock of behavior both suites already prove independently. The scenario table below is retained as an intent map so a future reader can see WHICH invariant is proven WHERE without hunting.**

| #   | Scenario intent                                                                                                                                                | Where proven                                                                                                                    |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| 17  | Live view converges to finalized view. `reduceWaveformSession` accumulates `amplitudeBuckets` on state during `accumulating`; the finalized artifact's `amplitudeBuckets` are `state.amplitudeBuckets.map(quantize8bit)` at terminal fold. | `bucketsToPathSegments.test.ts` "INV-B live vs finalized parity" — property over the 256-lattice + explicit raw-vs-quantized deep-equal. |
| 18  | Late metering is a no-op end-to-end; visualizer holds its last frame. Terminal IGNORE preserves state reference (INV-B6). | VOICE-004's `waveformSessionMachine.test.ts` blanket-IGNORE matrix (`Object.is` per cell). Renderer purity (INV-A) then guarantees the same buckets render the same segments. |
| 19  | Backgrounding freezes the visualizer. `AVAILABILITY_LOST` spread-preserves `amplitudeBuckets` reference in `toUnavailable`. | VOICE-004's `waveformSessionMachine.test.ts` `AVAILABILITY_LOST` scenario. Renderer purity closes the loop. |
| 20  | Multi-session reuse snaps back to centerline. `USER_RESET` from any terminal returns `initialWaveformSessionState()` with `amplitudeBuckets: []`. | VOICE-004's `waveformSessionMachine.test.ts` reset scenarios + `bucketsToPathSegments.test.ts` "empty buckets renders the centerline cue" (empty input → M/L stroke). |
| 21  | Visualizer works at the reducer's smallest legal outputs (`MIN_SAMPLES_FOR_FINALIZED` = 3 and `no_signal` at 2). | `bucketsToPathSegments.test.ts` canonical-shape scenarios cover 1-bucket, 2-bucket, 3-bucket, 8-bucket, 64-bucket, and 256-bucket cases; the small-count end is the bottom edge. VOICE-004's suite covers when each terminal actually fires. |
...
| 24  | **Reclassified per §13 orchestrator ruling 2026-08-04.** `appJsonPluginContract.test.ts` moves out of branch CI and into the runbook (operator adds the test file in the same commit as the `app.json` plugin edit + `npx expo install expo-audio`). Runbook step 3 spells out the exact test file body + the exact tuple text. Branch-CI scenario count for §7.3 drops from 4 to 3 (scenarios 22, 23, 25). |
...
3. **Add the `expo-audio` plugin tuple to `app.json` AND the `voice005AppJsonContract.test.ts` file — SAME commit as Step 2.** Add the exact tuple text (Appendix A of this runbook) into `expo.plugins` in `app.json`. Also create `__tests__/voice005AppJsonContract.test.ts` from Appendix E of this runbook. Expected: `git diff app.json` shows the additive `plugins` array; the new test file exists. Verification: `npx jest voice005AppJsonContract` exits 0; `npm run web:build` exits 0 (with `expo-audio` now installed the config resolver finds the plugin module). If-failed: Appendix C ("prebuild: cannot resolve plugin `expo-audio`" branch) — the plugin key mis-cased or the dep failed to install. Reversibility: `git checkout app.json && rm __tests__/voice005AppJsonContract.test.ts`. **Rationale:** the plugin tuple + contract test were moved out of the VOICE-005 branch per orchestrator ruling 2026-08-04 (§13). Adding the plugin to `app.json` on a branch without `expo-audio` installed breaks `npm run web:build` because the Expo config resolver walks `expo.plugins` eagerly and requires the module.
4. **Do NOT commit `package.json` / `package-lock.json`.** Runbook explicitly forbids `git add package*.json`. These are operator-local for this card.
5. **Optional: prebuild.** `npx expo prebuild --clean`. Expected: `ios/` and `android/` dirs appear (untracked). Verification: `grep -c 'NSMicrophoneUsageDescription' ios/*/Info.plist` returns `1` and the surrounding string matches `app.json` `plugins[0][1].microphonePermission` byte-for-byte. `grep -ci 'RECORD_AUDIO_BACKGROUND\|FOREGROUND_SERVICE_MICROPHONE' android/app/src/main/AndroidManifest.xml` returns `0`. If-failed: Appendix C. Reversibility: `git clean -fdx ios android`.
6. **Do NOT commit `ios/` / `android/`.** These are ephemeral CNG outputs. Runbook explicitly forbids `git add ios android` — freezes VOICE-002's future decision.
7. **Post-install branch CI.** `npm run typecheck && npm run lint && npm run test && npm run web:build`. Expected: all exit 0. Web bundle grep for `@shopify/react-native-skia` returns 0 hits (visualizer folder is Skia-free; if the component were committed and imported Skia statically, this would fail — this step proves the "pure renderer" promise). If-failed: back out.
8. **Reversal recipe (if the spike is aborted).** `git checkout main && git clean -fdx ios android node_modules && npm ci`. Expected: back at baseline; typecheck / lint / test / web:build all green; `app.json` `plugins` key gone; three deps gone. Reversibility floor confirmed.
9. **Cross-reference for the composition card.** Runbook links to `docs/designs/VOICE-005.md` §5 (spike component composition) so VOICE-005-composition / VOICE-007 has the intended shape without re-deriving it.
...
| `expo-audio` config plugin wired with doctrine mic-permission copy      | §8.3 (doctrine copy pinned by prose) + runbook step 3 (`app.json` edit + `voice005AppJsonContract.test.ts` add in same commit as `npx expo install expo-audio`)  | Operator's install action (per §13 orchestrator ruling 2026-08-04) — NOT this branch's CI |
...
## §13 — Implementer note: gap surfaced, ruling adopted (2026-08-04)

**Status:** RESOLVED. Orchestrator ruled option (a) on 2026-08-04 — defer the `app.json` plugin registration + `voice005AppJsonContract.test.ts` to the runbook (operator adds both in the same commit as `npx expo install expo-audio`). Pure-TS work re-implemented on `feat/voice-005-visualizer` after the design amendment landed. Doctrine copy stays pinned by prose in §8.3 + runbook Appendix A; runtime enforcement moves one card later. Retained below is the original gap report so future readers can see how the design converged.
...
### 13.4 What implementer initially committed (superseded by the ruling)

The first implementer pass committed ONLY this note (commit `f8c3bdd2`, "cannot proceed") per the strict role-rule "commit that change alone". All prior implementer work (pure-TS visualizer module, four adapter/property tests, two doctrine tests, one runbook file, and the `app.json` plugin edit) was reset from the worktree before that commit.

The pure-TS work was fully written and green through gates 1-4 in that first pass. Gate 5 (`web:build`) went red only because of the `app.json` plugin addition; with that stashed, gate 5 was green. This scoped the gap precisely to the plugin registration.

### 13.5 What the orchestrator ruled

Orchestrator adopted **option (a)** on 2026-08-04: defer the `app.json` plugin registration + `voice005AppJsonContract.test.ts` to the runbook. The rest of the design's `commit_now` file manifest was left intact. Reasoning:

- Keeps this branch fully agent-safe (all 5 gates green with no `app.json` change).
- Keeps the operator's install step coherent (install + `app.json` plugin edit in the same coordinated action).
- Avoids the design-vs-Expo-resolver conflict entirely (no dynamic `app.config.js` migration needed).
- Doctrine copy stays pinned by prose in §8.3 + runbook Appendix A (two written sources); runtime enforcement moves one card later.

### 13.6 What implementer did after the ruling

- Amended the design in a second commit: this §13 update, §0.1 row 7 (new deferral row), §1 file manifest (app.json + contract test reclassified `operator_step_only`), §7.3 scenario 24 (marked reclassified), §8.2 (new step 3 with the exact tuple text + contract-test body; runbook step numbering shifted), §12 acceptance mapping row 7 (ratifier changed to "operator's install action").
- Re-implemented the pure-TS work (5 modules + 1 fixture + 5 test files, minus `voice005AppJsonContract.test.ts`) in a subsequent commit on the same branch. All 5 gates ended green.
- Runbook Appendix E carries the exact contract-test body the operator adds when they perform runbook step 3.

Cross-reference: the re-implementation commit sha appears in the branch history AFTER this design amendment commit. Files touched on `feat/voice-005-visualizer` beyond the two design commits: `docs/runbooks/VOICE-005-install-runbook.md`, `src/features/voice/visualizer/**`, and `__tests__/voice005*` (minus `voice005AppJsonContract.test.ts`) plus `__tests__/bucketsToPathSegments.test.ts` / `quantizeBucketsForRender.test.ts` / `svgPathAdapter.test.ts` / `skiaPathAdapter.test.ts`.
