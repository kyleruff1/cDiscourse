...
- **Acceptance:** `BATCH_SIZE = 16`, split-threshold = 20; ≤20 families → exactly 1 byte-identical batch (A/B/C/E/F unchanged, proven against the real builder); >20 proven with a synthetic fixture; per-batch response passes the UNCHANGED validator; a 21-entry single response still rejects (cap intact); one run per (argument, family), positives unioned, no double-count; partial-persist + leak-safe `failure_detail {batchIndex,batchTotal,reason}` on the direct path; family-granularity retry on the drainer; no schema-version bump; batch metadata never on the wire; `engine.ts` untouched; full `mcpOneTwoOneBReadOnlyBoundary` suite green.
- **Tests:** +66 net Jest (3 new suites) + intent-preserving source-scan updates; mcp-server Deno 1250/0.
