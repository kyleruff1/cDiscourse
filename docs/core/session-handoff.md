...
12. **Submit path is fingerprint-based (Stage 5.5.3+).** `composerSubmit.ts` contains the authoritative idempotency helpers: `createSubmissionFingerprint`, `shouldReuseClientSubmissionIdForRetry`, `getOrCreateClientSubmissionId`, `buildSubmitArgumentPayload`. Earlier helpers in `composerHelpers.ts` (`shouldReusePendingSubmission`, `shouldCreateNewClientSubmissionId`) predate the fingerprint approach and are less complete — do not use them for submit retry logic. `buildSubmitArgumentPayload` must never include `author_id`, `depth`, `status`, or `server_validation`.
