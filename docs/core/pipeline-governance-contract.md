...
> **Provenance.** §0–§2 below are transcribed verbatim from the operator's authoring context; §3–§8 were drafted by Claude Code from the card's references, §0–§2's own forward-references, and the demonstrated governance behavior of the 2026-06-02/03 cutover cards. The contract was **operator-reviewed and revised** (the §5 `mcp-server` merge-vs-deploy split and the §2/§5/§6 "agent never merges" clarification), then **operator-ratified, committed, and merged in PR #449** (`cedf97e`, 2026-06-03). This is the **binding governance contract** — not a draft.
