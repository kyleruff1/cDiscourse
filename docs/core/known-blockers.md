...
**Status:** Non-blocking; R3 disambiguation **RESOLVED 2026-06-02**; F-hardening **implemented + merged** (PR #443, `f529edb`) → remaining step is the operator Deno Deploy push + smoke.
A low transient floor (~2–4% per-attempt `provider_server_error`) is absorbed by the 4-attempt retry budget. The ONE isolated Family-F (`critical_question`) cell (argId `9ef5aab5…`, a **synthetic** smoke-tagged qualification-burst arg — `organic_non_smoke_routed_args = 0`) that exhausted all 4 attempts was disambiguated via the Deno Deploy R3 `boolean_observation_tool_error` log: it is a **deterministic packet-shape residual** (`validation_failed` on `evidenceSpan.unstated_assumption`; 5/5 `packet_invalid`, 0 ban-list, 61 healthy Anthropic 200s), **NOT** a provider-side 5xx — the queue's `provider_server_error` sub_reason is an Edge-adapter bucketing artifact. It did **not** change the `PASS-STAGE-1-PLUMBING / INSUFFICIENT-ORGANIC-VOLUME` verdict (organic = 0). **Remaining follow-up:** the fix (design `docs/designs/OPS-MCP-FAMILY-F-UNSTATED-ASSUMPTION-SHAPE-TUNING.md`, PR #441) extends the PR #421/#423 rule-6 RAWKEY-SHAPE REINFORCEMENT to `unstated_assumption`. **Code is implemented + merged to `main` (PR #443, `f529edb`) but NOT yet live** — `mcp-server/` deploys separately to Deno Deploy (merge-auto-deploy covers `supabase/functions/` only). **Operator step:** push `cdiscourse-mcp-server` to Deno Deploy, then hosted smoke → 23/23 + N=1 canary + N=8 burst to confirm `evidenceSpan.unstated_assumption` no longer dead-letters (read the now-live `failure_detail` on any residual). REQUIRED before any 5% ramp. Full evidence: § 12 of the Stage-1 cutover audit + `docs/reviews/OPS-MCP-FAMILY-F-UNSTATED-ASSUMPTION-SHAPE-TUNING.md`.
