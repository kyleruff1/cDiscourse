...
|---|---|---|
| 1 — "Start an argument" entry, no game/debate copy | RULE — terminology | **New: QOL-035** (live tab already correct) |
| 2 — Compose: optional title, blank → root excerpt | Existing — debate title (Stage 6.1.8) | Covered |
| 2 — Visibility chosen at compose (Public default) | RULE — room visibility | **New: QOL-039** |
| 2 — Invite by email | IX — invite path | **New: QOL-038** |
| 3 — Root claim + classification tags | Existing — TL-002 (root marker), VG-001 (grammar), RULE-005 (channels) | Covered |
| 4 — Invite email; no "debate/game" subject | IX — invite path | **New: QOL-038** |
| 4 — No-account → signup → return; account → auth → room | IX — auth deep link | **New: QOL-038** |
| 5 — Invited respondent skips observer/side modal | Existing — Stage 6.4 seamless entry, TL-001 | Covered |
| 6 — Itemized concession list + main refutation composer | RULE — concession UI | **New: QOL-041** |
| 7 — "New response" notification → returns to room | IX — notifications | **New: QOL-040** |
| 8 — Concession acceptance gradient (5 levels) | RULE — concession UI | **New: QOL-041** |
| 8 — Required clarification on disagreement | Existing — RULE-004 (advisory review); extended by QOL-041 | Partial |
| 9 — "Concession challenged" notification | IX — notifications | **New: QOL-040** |
...
| 13 — Chime-in notification to both primaries | IX — notifications | **New: QOL-040** |
...
| 15 — Public → private transition | RULE — room visibility | **New: QOL-039** |
| 15 — "Made private" / "chime-in rejected" observer notifications | IX — notifications | **New: QOL-040** |
| 16–17 — Mainline concession + refutation nodes | Existing — VG-001, RULE-005, point-standing-economy | Covered |
| 18 — "Touché" partial-resolution control | RULE — concession UI / reactions | **New: QOL-041** (reaction affordance) |
| 18 — Tangent flag → diagonal branch | Existing — BR-003 (tangent routing), BR-004 (`tangent_diagonal`) | Covered |
| 19 — Tangent-branch response + fist-bump reaction | RULE — reactions | **New: QOL-041** (fist-bump) |
| 20 — Settlement → grey, locked board | Existing — room status / lock model | Covered |
| 21 — Linked prior argument as context | RULE — cross-room link | **New: QOL-042** |

## Scenario 2 — Band / rent / private evidence

| Story moment | Maps to | Status |
|---|---|---|
| 1 — Private argument from creation | RULE — room visibility | **New: QOL-039** |
| 1 — Invite by email into a private room | IX — invite path | **New: QOL-038** |
| 2 — Root claim, payment-dispute tags, evidence reference | Existing — EV-001 (evidence object), RULE-005 (channels) | Partial — see QOL-036 |
| 3 — Private invite → auth → private room, never listed | IX — auth deep link + RULE visibility | **New: QOL-038 + QOL-039** |
| 4 — Private room marked Private; evidence panel available | RULE — visibility; EV — panel | **New: QOL-039**; EV-002/005 follow-up |
| 5 — "Attach evidence" toggle; evidence as an object | Existing — EV-001; toggle UI extends it | Partial — EV-001 follow-up |
| 5 — Payment metadata (amount, date, redacted payer/payee, note, claimed month) | EV — evidence metadata | **New: QOL-036** |
| 6 — Structured evidence response (Accept / Dispute applicability / …) | EV — applicability flow | **New: QOL-037** |
| 6 — Required clarification on a non-Accept response | Existing — RULE-004; extended by QOL-037 | Partial |
...
| 8 — "Agree with caveat" on evidence | EV — applicability flow | **New: QOL-037** |
| 9 — "Ask for source" → evidence-debt marker, "Evidence requested" | Existing — EV-002, EV-003 | Covered |
| 10 — Evidence supplied → status improves | Existing — EV-003 (debt resolution); status extends via QOL-037 | Partial |
| 11 — Concession list ("I concede …") | RULE — concession UI | **New: QOL-041** |
| 12 — "Disagree based on context" + attach evidence | EV — applicability flow; QOL-037 / QOL-041 | **New** |
| 13 — "Accept evidence" + summary; "Confirm resolution" → settled lock | Existing — room status / lock; QOL-037 for the evidence-accept action | Partial |
| 14 — Linked prior **private** argument with access check | RULE — cross-room link | **New: QOL-042** |
| All — invite / response / source / settled notifications | IX — notifications | **New: QOL-040** |
...
|---|---|---|---|
| **QOL-034** | Storyboard canon + storyline-narrative-officer skill | Project Mgmt | P1 / S / 6.6 |
| **QOL-035** | User-facing terminology scrub: no "game", prefer "Argument" | Rules UX | P1 / M / 6.6 |
| **QOL-036** | Payment / screenshot evidence metadata object | Evidence | P1 / M / 6.7 |
| **QOL-037** | Evidence applicability dispute flow | Evidence | P1 / M / 6.7 |
| **QOL-038** | Invite → signup/auth → argument-room return path | Interaction | P1 / L / 6.7 |
| **QOL-039** | Public ↔ private room visibility transition rules | Rules UX | P1 / M / 6.7 |
| **QOL-040** | Invite & response notification lifecycle | Interaction | P2 / L / 6.7 |
| **QOL-041** | Concession acceptance gradient + concession-list UI | Rules UX | P1 / M / 6.6 |
| **QOL-042** | Linked prior argument reference (context / evidence) | Rules UX | P2 / M / 6.8 |
...
2. The new `QOL-034 … QOL-042` cards are already in
